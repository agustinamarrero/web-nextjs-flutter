    ...
    async function d(n, e, r) {
      ...
      let t,
        i = new Promise((o, s) => {
          t = setTimeout(() => {
            s(
              new Error(`${r} took more than ${e}ms to resolve. Moving on.`, {
                cause: d,
              })
            );
          }, e);
...
Read more: https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts`)),
        ...
        let {
            serviceWorkerVersion: r,
            serviceWorkerUrl: t = `${l}flutter_service_worker.js?v=${r}`,
            timeoutMillis: i = 4e3,
          } = e,
          o = t;
      ...
      async _getNewServiceWorker(e, r) {
        ...
        if (e.active.scriptURL.endsWith(r))
          return (
            console.debug("Loading from existing service worker."), e.active
          );
        {
          let t = await e.update();
          return (
            console.debug("Updating service worker."),
            t.installing || t.waiting || t.active
          );
        }
      }
      async _waitForServiceWorkerActivation(e) {
        ...
        return new Promise((r, t) => {
          e.addEventListener("statechange", () => {
            e.state == "activated" &&
              (console.debug("Activated new service worker."), r());
          });
        });
      }
    }
    ...
    class v {
      async loadEntrypoint(e) {
        ...
        let s = new f();
        return (
          s.setTrustedTypesPolicy(i.policy),
          (this.didCreateEngineInitializer =
            s.didCreateEngineInitializer.bind(s)),
          s.loadEntrypoint(t)
        );
      }
    }
